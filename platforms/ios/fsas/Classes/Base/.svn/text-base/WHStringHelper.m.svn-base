...
+(NSString*)timeHoursDate:(NSDateComponents*)date{
    NSString* hours=[self prefixDate:date.hour];
    NSString* mins=[self prefixDate:date.minute];
    
    NSString* dd=[NSString stringWithFormat:@"%@:%@",hours,mins];
    return dd;
}
...
//HH:mm
+ (NSString *)shortDateHoursStamp:(double)timestamp {
    
   return  [self timeHoursDate:[self dateComponentByTimeStamp:timestamp]];
}
