...
@property(copy,nonatomic)NSString* MA5Price;
@property(copy,nonatomic)NSString* MA10Price;
@property(copy,nonatomic)NSString* MA20Price;
@property(copy,nonatomic)NSString* MA60Price;
@end
...
-(void)initSet{
 ...
 //   self.yHeight = self.frame.size.height;
    ...
    if (iPhone6Plus) {
        self.kLinePadding = 1*WIDTH6TO6P;
        self.kLineWidth = 5*WIDTH6TO6P;
    }
    if (iPhone6) {
...
-(void)start{
    [self drawBox];
    thread = [[NSThread alloc] initWithTarget:self selector:@selector(drawKLine) object:nil];
    [thread start];
}
...
-(void)drawBox{
    // 画个k线图的框框
    if (_mainboxView==nil) {
        _mainboxView = [[UIView alloc] initWithFrame:CGRectMake(self.xPozition,self.yPozition, self.xWidth, self.yHeight)];
...
//        mainboxView.layer.borderColor = [UIColor colorWithHexString:@"#444444" withAlpha:1].CGColor;
        _mainboxView.backgroundColor = [UIColor clearColor];
        _mainboxView.layer.borderColor = [UIColor colorWithRed:68/255.0 green:68/255.0 blue:68/255.0 alpha:1].CGColor;
        _mainboxView.layer.borderWidth = 0;
        ...
        [_mainboxView addGestureRecognizer:pinchGesture];
...
-(void)showCurrentPrice{
    ...
    if (MA5==nil) {
        ...
        MA5.font = self.font;
        MA5.text = [NSString stringWithFormat:@"MA5:%.2f",[self.MA5Price floatValue]];
        MA5.textColor = FIVE_LINE_COLOR;
        [MA5 sizeToFit];
        [self addSubview:MA5];
    }
    //
    ...
    if (MA10==nil) {
        ...
        MA10.font = self.font;
        MA10.text = [NSString stringWithFormat:@"MA10:%.2f",[self.MA10Price floatValue]];
        MA10.textColor = TEN_LINE_COLOR;
        [MA10 sizeToFit];
    ...
    if (MA20==nil) {
        ...
        MA20.font = self.font;
        MA20.text = [NSString stringWithFormat:@"MA20:%.2f",[self.MA20Price floatValue]];
        MA20.textColor = TWENTY_LINE_COLOR;
        [MA20 sizeToFit];
    ...
    if (MA60==nil) {
        MA60 = [[UILabel alloc] initWithFrame:CGRectMake(MA20.frame.origin.x+MA20.frame.size.width+10, 0, 30, 15)];
        MA60.backgroundColor = [UIColor clearColor];
        MA60.font = self.font;
        MA60.text = [NSString stringWithFormat:@"MA60:%.2f",[self.MA60Price floatValue]];
        MA60.textColor = SIXTY_LINE_COLOR;
        [MA60 sizeToFit];
        [self addSubview:MA60];
    }
...
#pragma mark 画k线
-(void)drawKLine{
    CGSize rect = [[UIScreen mainScreen].currentMode size];
    if (iPhone5) {
        self.kLineWidth = 3.7;
        self.kLinePadding = 1;
    }
    if (iPhone6Plus) {
        self.kLinePadding = 1;
        self.kLineWidth = 5.30;
    }
    if (iPhone6) {
        self.kLineWidth = 4.74;// k线实体的宽度
        self.kLinePadding = 1; // k实体的间隔
    }
...
//    self.data = getdata.data;
    ...
    WPNSLOG(@"当前：%lu",(unsigned long)self.data.count);
    // 开始画K线图
...
//    [self changeMaxAndMinValue];
   //  平均线
    CGFloat padValue = (getdata.maxValue - getdata.minValue) / 5;
...
//        leftTag.textColor = [UIColor colorWithHexString:@"#cccccc" withAlpha:1];
        leftTag.textColor = [UIColor colorWithRed:204/255.f green:204/255.f blue:204/255.0 alpha:1];
        leftTag.font = self.font;
        leftTag.textAlignment = UITextAlignmentRight;
        leftTag.backgroundColor = [UIColor clearColor];
        //[leftTag sizeToFit];
  //      [_mainboxView addSubview:leftTag];
...
//    NSArray *ktempArray = [self changeKPointWithData:getdata.data]; // 换算成实际每天收盘价坐标数组
    NSArray* ktempArray = [self changeKPointWithData:self.data];      //将自身股票数据换算成每天收盘价坐标数据   data由上一级传入
    lines *kline = [[lines alloc] initWithFrame:CGRectMake(0, 0, _mainboxView.frame.size.width, _mainboxView.frame.size.height)];
    kline.points = ktempArray;
    kline.lineWidth = self.kLineWidth;
    kline.isK = YES;
    [_mainboxView addSubview:kline];
    [lineArray addObject:kline];
    
    
    [self drawMAWithIndex:5 andColor:@"#32ABF6"];
    [self drawMAWithIndex:10 andColor:@"#FFD372"];
    [self drawMAWithIndex:20 andColor:@"#FF1515"];
    [self drawMAWithIndex:60 andColor:@"#C25AFF"];
...
-(void)drawMAWithIndex:(int)index andColor:(NSString*)color{
    NSArray *tempArray = [self changePointWithData:self.MAData andMA:index]; // 换算成实际坐标数组
    lines *line = [[lines alloc] initWithFrame:CGRectMake(0, 0, _mainboxView.frame.size.width, _mainboxView.frame.size.height)];
    line.color = color;
    line.points = tempArray;
    line.isK = NO;
    [_mainboxView addSubview:line];
...
-(void)gestureRecognizerHandle:(UILongPressGestureRecognizer*)longResture{
    ...
    touchViewPoint = [longResture locationInView:_mainboxView];
    // 手指长按开始时更新一般
...
-(void)updateNib{
    ...
    if (movelineone==Nil) {
        ...
        [_mainboxView addSubview:movelineone];
        movelineone.hidden = YES;
    }
    if (movelinetwo==Nil) {
        movelinetwo = [[UIView alloc] initWithFrame:CGRectMake(0,0, _mainboxView.frame.size.width,0.5)];
        movelinetwo.backgroundColor = [UIColor whiteColor];
        movelinetwo.hidden = YES;
        [_mainboxView addSubview:movelinetwo];
    }
    if (movelineoneLable==Nil) {
        ...
        movelineoneLable.textAlignment = NSTextAlignmentRight;
        movelineoneLable.alpha = 0.8;
        movelineoneLable.hidden = YES;
        [_mainboxView addSubview:movelineoneLable];
    }
    if (movelinetwoLable==Nil) {
        ...
        movelinetwoLable.textAlignment = NSTextAlignmentRight;
        movelinetwoLable.alpha = 0.8;
        movelinetwoLable.hidden = YES;
        [_mainboxView addSubview:movelinetwoLable];
    }
    
    movelineone.frame = CGRectMake(touchViewPoint.x,0, 0.5,
                                   bottomBoxView.frame.size.height+bottomBoxView.frame.origin.y);
    movelinetwo.frame = CGRectMake(0,touchViewPoint.y, _mainboxView.frame.size.width,0.5);
    CGRect oneFrame = movelineone.frame;
    oneFrame.size = CGSizeMake(50, 12);
...
-(NSArray*)changePointWithData:(NSArray*)data andMA:(int)MAIndex{
    ...
    NSArray *MAData = [[NSMutableArray alloc]init];
    float temp = 0.0;
    CGFloat PointStartX = 0.0f; // 起始点坐标
    NSInteger count = [data count];

        for (int i = 0; i<count-MAIndex; i++) {
            temp = 0;
            for (int j = i; j<i+MAIndex; j++) {
                temp += [[data objectAtIndex:j] floatValue];
            }
            temp = temp/MAIndex;
            [averageData addObject:[NSString stringWithFormat:@"%f",temp]];
        }
    
    
    if (MAIndex == 5) {
        self.MA5Price = [averageData lastObject];
    }
    if (MAIndex == 10) {
        self.MA10Price = [averageData lastObject];
    }
    if (MAIndex == 20) {
        self.MA20Price = [averageData lastObject];
    }
    if (MAIndex == 60) {
        self.MA60Price = [averageData lastObject];
    }
    ...
    if ([averageData count] >= number) {
        NSRange range = NSMakeRange([averageData count] - number, number);
        MAData = [averageData subarrayWithRange:range];
        PointStartX += 0;
    }
    else{
        MAData = averageData;
        PointStartX += (self.kLinePadding+self.kLineWidth)*(number - [MAData count]);   //得到第一个点的位置
    }
    for (int i = 0;i<[MAData count];i++) {
   //     CGFloat currentValue = [[item objectAtIndex:MAIndex] floatValue];// 得到前五天的均价价格
        // 换算成实际的坐标
        CGFloat currentPointY = self.yHeight - (([[MAData objectAtIndex:i]floatValue] - [self.lowPrice floatValue]) / ([self.highPrice floatValue] - [self.lowPrice floatValue]) *self.yHeight);
...
//        CGFloat yViewHeight = _mainboxView.frame.size.height ;// y的实际像素高度
        ...
        CGFloat yHeight = [self.highPrice floatValue] - [self.lowPrice floatValue];      //y的价值高度
        CGFloat yViewHeight = _mainboxView.frame.size.height;    //y的实际像素高度
        
        // 换算成实际的坐标
        CGFloat heightPointY = yViewHeight * (1 - (heightvalue - [self.lowPrice floatValue]) / yHeight);
        CGPoint heightPoint =  CGPointMake(PointStartX, heightPointY); // 最高价换算为实际坐标值
        CGFloat lowPointY = yViewHeight * (1 - (lowvalue - [self.lowPrice floatValue]) / yHeight);;
        CGPoint lowPoint =  CGPointMake(PointStartX, lowPointY); // 最低价换算为实际坐标值
        CGFloat openPointY = yViewHeight * (1 - (openvalue - [self.lowPrice floatValue]) / yHeight);;
        CGPoint openPoint =  CGPointMake(PointStartX, openPointY); // 开盘价换算为实际坐标值
        CGFloat closePointY = yViewHeight * (1 - (closevalue - [self.lowPrice floatValue]) / yHeight);;
...
-(void)isKPointWithPoint:(CGPoint)point{
    ...
    for (NSArray *item in pointArray) {
        ...
        if (itemX==pointX || point.x-itemX<=self.kLineWidth/2) {
            ...
            if ((_mainboxView.frame.size.width - itemPointX)<movelineoneLable.frame.size.width/2) {
                oneLableX = -(movelineoneLable.frame.size.width/2 - (_mainboxView.frame.size.width - itemPointX));
            }
            ...
            // 如果滑动到了左半边则提示向右跳转
            if ((_mainboxView.frame.size.width - itemPointX) > _mainboxView.frame.size.width/2) {
                twoLableX = _mainboxView.frame.size.width - movelinetwoLable.frame.size.width;
            }else{
                twoLableX = 0;
            }
