...
@interface QUPhoneTextFieldHelper()<UITextFieldDelegate>
@property(nonatomic,weak)QUPhoneTextField* textField;
@end

@implementation QUPhoneTextFieldHelper
-(instancetype)initWithPhoneTextField:(QUPhoneTextField*)textField{
    self=[super init];
...
@interface QUPhoneTextField () <UITextFieldDelegate>
{
    NSString    *previousTextFieldContent;
    UITextRange *previousSelection;
}

@property(nonatomic,strong)QUPhoneTextFieldHelper* helper;
@end

@implementation QUPhoneTextField
...
-(void)setupFormat
{
    self.helper=[[QUPhoneTextFieldHelper alloc] initWithPhoneTextField:self];
...
{
    if([[QUPhoneTextField removeFormat:textField.text] length]<=11)
    {
        previousTextFieldContent = textField.text;
        previousSelection = textField.selectedTextRange;
        
        if([textField.text length]<13)
            self.bInputCompletedContent=NO;  // 如果输入位数小于13，则说明在删除操作，状态输入不完整
        else
            self.bInputCompletedContent=YES; // 输入完整
    }
    ...
    if ([cardNumberWithoutSpaces length] > 11) {
        ...
        kBeginSetTextField(reformatAsCardNumber);
        [textField setText:previousTextFieldContent];
        kEndSetTextField(reformatAsCardNumber);
