...
//编辑地址
var editAddress=function(newAddress_data){
    //newAddress_data.user_id="13621804326";
    hlp.log("before call shopping-car.js editAddressList");
    svc.editAddressList(newAddress_data, function (editAddress_res) {
        hlp.log("inside call shopping-car.js editAddressList");
        if (editAddress_res.status == "SUCCESS") {
            hlp.log(editAddress_res.message);
            $.afui.loadContent("#submitOrder");
        } else {
            hlp.log(editAddress_res.message);
            hlp.myalert("修改地址失败");
