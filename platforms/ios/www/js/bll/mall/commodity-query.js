$(function () {
            ...
            if (r.status == "SUCCESS") {
                ...
                var setNextPageTimerFunc = function(parentDiv) {
                    ...
                    if (timerId) {
                        ...
                        newId = StartCountDown(new Date(promoBeginTime).getTime(), function (remainTime) {
                            $("#detailSecKill").html(remainTime.string);
                        });
                    } else {
                        newId = StartCountDown(new Date(testEndTime).getTime(), function (remainTime) {
                            $("#detailSecKill").html(remainTime.string);
                        });
                    }
                ...
                $("#secKillBannerList").text($(r.data.ad_img).text());
    ...
    $("#secKill").on("panelunload", function () {
        ...
        svc.getCommodityComment(goods_sn,function(r){
            ...
            if(r.status == "SUCCESS"){
                    ...
                    if (rank == 0 || rank == NaN) {
                        ...
                        switch (rank){
                            case 5:
                                ...
                                break;
                            case 4:
                                r.data.comment[i]["rankLevel"] = "比较满意";
                                break;
                            case 3:
                            ...
                            case 2:
                                ...
                                break;
                            case 1:
                                r.data.comment[i]["rankLevel"] = "相当差";
                                break;
                        }
                    }

                    var currentId = r.data.comment[i].comment_id;
                }
                ...
                for (var i = 0; i < r.data.comment.length; i++) {
                    ...
                    var currentId = r.data.comment[i].comment_id;
                    var obj = $('.'+currentId);
                    if(r.data.comment[i].append_comment.length == 0){
                        obj.find(".zj")[0].remove();
                        obj.find(".zj")[0].remove();
                     }

                    if(r.data.comment[i].answer == null || r.data.comment[i].answer == ""){
                        obj.find(".gf")[0].remove();
                        obj.find(".gf")[0].remove();
                    }
    ...
    $("#commodityDetails1").on("panelunload", function () {
...
var setMsFlg = function () {
    ...
    $.each(secKill, function (i) {
        ...
        if (curTime >= promoBeginTime && curTime < promoEndTime) {
            ...
            var timerId = StartCountDown(new Date(testBeginTime).getTime(), function (remainTime) {
                button.html(remainTime.string);
            });
